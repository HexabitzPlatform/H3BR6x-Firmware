...
//					button[i].state = PRESSED;// Record a PRESSED event. This event is always reset on next tick.
				...
				/* Reset ReleaseCounter if needed - to avoid masking PressCounter on NO switches */
			...
			if(ReleaseCounter[i] < BOS.Buttons.Debounce){
				...
				/* Reset PressCounter if needed - to avoid masking ReleaseCounter on NC switches */
				if(PressCounter[i] > BOS.Buttons.Debounce)
					PressCounter[i] =0;
...
BOS_Status AddPortButton(ButtonType_e buttonType,uint8_t port){
	...
	/* 2. De-initialize UART (only if module is initialized) */
	if(bosInitialized){
	...
	if(!res){
		...
		/* This is same port and same type, do not update */
		if(((temp8 >> 4) == port) && ((temp8 & 0x0F) == buttonType))
			return BOS_OK;
...
BOS_Status RemovePortButton(uint8_t port){
	...
	res =EE_ReadVariable(_EE_BUTTON_BASE + 4 * (port - 1),&temp16);
	/* This variable exists, reset all to zeros */
	if(!res)
	{
...
BOS_Status SetButtonEvents(uint8_t port,ButtonState_e buttonState,uint8_t mode){
	...
	if(!res){
		...
		if((uint8_t )(temp16) != Button[port].Event){
